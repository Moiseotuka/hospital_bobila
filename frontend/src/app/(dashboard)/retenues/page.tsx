...
import { retenueService } from "@/services/retenue.service"
...
export default function RetenuesPage() {
  ...
  const [editItem, setEditItem] = useState<Retenue | null>(null)
  const [deleteId, setDeleteId] = useState<number | null>(null)
  const debouncedSearch = useDebounce(search, 300)

  const { data, isLoading } = useQuery({
    queryKey: ["retenues", debouncedSearch, typeFilter],
    queryFn: () => retenueService.getRetenues({
  ...
  const createMut = useMutation({
    mutationFn: (d: any) => retenueService.createRetenue(d),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["retenues"] }); toast.success("Retenue créée"); setDialogOpen(false) },
    onError: (e: any) => toast.error(e?.response?.data?.message || "Erreur"),
  })
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => retenueService.updateRetenue(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["retenues"] }); toast.success("Retenue mise à jour"); setDialogOpen(false); setEditItem(null) },
    onError: (e: any) => toast.error(e?.response?.data?.message || "Erreur"),
  })
  const deleteMut = useMutation({
    mutationFn: (id: number) => retenueService.deleteRetenue(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["retenues"] }); toast.success("Retenue supprimée"); setDeleteId(null) },
    onError: (e: any) => toast.error(e?.response?.data?.message || "Erreur"),
  })

  const items = data?.data || data || []
  const types = [...new Set(items.map((p: Retenue) => p.type).filter(Boolean))] as string[]

  const columns: ColumnDef<Retenue>[] = [
    { accessorKey: "code", header: "Code" },
    { accessorKey: "nom", header: "Nom" },
    {
      accessorKey: "type",
      header: "Type",
      cell: ({ row }) => <Badge variant="warning">{row.original.type || "N/A"}</Badge>,
    },
    {
      id: "valeur",
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader title="Retenues" description="Gestion des retenues" actions={
        <Button onClick={() => { setEditItem(null); setDialogOpen(true) }}><Plus className="h-4 w-4 mr-2" /> Nouvelle Retenue</Button>
      } />
      <Card>
...
function RetenueDialog({ open, onOpenChange, editItem, onSubmit, isPending }: {
  open: boolean; onOpenChange: (o: boolean) => void; editItem: Retenue | null; onSubmit: (d: FormData) => void; isPending: boolean
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) reset() }}>
      <DialogContent>
        <DialogHeader><DialogTitle>{editItem ? "Modifier" : "Nouvelle"} Retenue</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Type *</Label>
            <Input {...register("type")} placeholder="Ex: cnss, ipr..." className={errors.type ? "border-destructive" : ""} />
